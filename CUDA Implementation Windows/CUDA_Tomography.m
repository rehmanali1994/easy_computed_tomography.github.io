...
orig_object = im2double(rgb2gray(imread('../images/frontalplane.jpg')));
...
system(['sinogram.exe ', num2str(numAngles), ' theta.txt ', ...
...
system(['filtBackproj.exe ', num2str(numAngles), ' theta.txt ', ...
	num2str(-rmax), ' ', num2str(rmax), ' ', num2str(Nr), ' sg.txt ', ...
	num2str(min(x)), ' ', num2str(max(x)), ' ', num2str(Nx), ' ', ...
...
system('del img.txt');
system('del recon.txt');
system('del sg.txt');
system('del theta.txt');
