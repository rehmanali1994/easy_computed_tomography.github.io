...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  double *x = (double *)mxGetData(prhs[3]);
	...
	sg = (float*)malloc(numAngles*numSensors*sizeof(float));
	for (int jj = 0; jj < numAngles; jj++) {
		for (int ii = 0; ii < numSensors; ii++) {
			sg[ii + numSensors*jj] = (float)sinogram[ii + numSensors*jj];
		}
	}
	...
	mParams.srcPtr = make_cudaPitchedPtr(sg, numSensors*sizeof(float), numSensors, 1);
